...
async def generate_and_play_tts(text):
    global is_ai_speaking, auto_resume_mic
    ...
    try:
        ...
        # 3. AUTO-RESUME CHECK (If Godot requested it)
        if auto_resume_mic:
            global is_user_typing
            is_user_typing = False
            auto_resume_mic = False # Reset flag
            print(">> Mic Auto-Resumed (After Speech)")
...
def run_tts_listener():
    global is_user_typing, auto_resume_mic
    while True:
        try:
            ...
            if text:
                if text == "__SYS__PAUSE_MIC":
                    is_user_typing = True
                    # If we pause, we should cancel any pending auto-resume
                    auto_resume_mic = False 
                    print(">> Mic Paused (Typing/Chat Mode)")
                elif text == "__SYS__RESUME_MIC":
                    is_user_typing = False
                    print(">> Mic Resumed")
                elif text == "__SYS__AUTO_UNPAUSE":
                    auto_resume_mic = True
                    print(">> Auto-Unpause Armed")
                
                # --- NEW COMMAND HANDLING ---
                elif text.startswith("__CMD__"):
                    cmd_content = text.replace("__CMD__", "")
                    print(f"Lia Command: {cmd_content}")
                    
                    # 1. Execute Action
                    success, message = action_engine.execute_command(cmd_content)
                    
                    # 2. Speak Result (Feedback Loop)
                    if message:
                        asyncio.run(generate_and_play_tts(message))
...
def run_mic_listener():
    with sr.Microphone() as source:
        ...
        while True:
            ...
            try:
                # Reduced timeout to catch shorter phrases, increased phrase limit slightly
                ...
                if text and text.strip() != "":
                    ...
                    print("... (Sent) ...")
                    # allow immediate follow-up or processing
                    time.sleep(0.1) 
